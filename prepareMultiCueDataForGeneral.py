...
class PrepareMultiCueDataSamples(object):
    ...
    def __init__(self, config):
        ...
        self.test_lst, self.test_num = self.get_sample_lst(
            sample_lst_path[2])
        speaker_lst_path = config.SPEAKER_LIST
        self.spk_train_set, self.spk_train_dict = self.get_spk_set(
            os.path.join(config.DATA_PATH, speaker_lst_path[0]))
        self.spk_valid_set, self.spk_valid_dict = self.get_spk_set(
            os.path.join(config.DATA_PATH, speaker_lst_path[1]))
        self.spk_test_set, self.spk_test_dict = self.get_spk_set(
            os.path.join(config.DATA_PATH, speaker_lst_path[2]))
        noise_lst_path = config.NOISE_PATH
        # self.noise_lst, self.noise_num = self.get_sample_lst(
        #     noise_lst_path[0])
    ...
    def get_masked_cues(self, infer_cue_dict, azimuth, spk1_visual, other_spk_visual, spk1_ref_vp, other_ref_vp, spk1_ref_wav, other_ref_wav):
        # for interfere
        if 'azimuth' in infer_cue_dict['interfere']:
            azimuth = float(
                azimuth) + random.uniform(-self.azimuth_noise, self.azimuth_noise)
        if 'visual' in infer_cue_dict['interfere']:
            missing_rate = random.uniform(0, 0.5)
            missing_begin = int(random.uniform(0, 1 - missing_rate) * len(spk1_visual))
            missing_offset = int(missing_rate * len(spk1_visual))
            spk1_visual[missing_begin:missing_begin+missing_offset] = 0
        if 'voiceprint' in infer_cue_dict['interfere']:
            pass
            # noise_file = random.choice(self.noise_lst)[0]
            # noise_wav, wav_rate = sf.read(noise_file)
            # noise_wav = self.preprocess_data(noise_wav, wav_rate)
            # noise_wav = list(noise_wav)
            # if len(noise_wav) > len(spk1_ref_wav):
            #     noise_begin = random.choice(range(len(noise_wav) - len(spk1_ref_wav) + 1))
            #     noise_wav = noise_wav[noise_begin:noise_begin + len(spk1_ref_wav)]
            # noise_wav.extend(
            #     np.zeros(len(spk1_ref_wav) - len(noise_wav)))
            # noise_wav = self.random_shift_data(noise_wav)
            # noise_wav = np.array(noise_wav)
            # noise_db_rate = np.random.uniform(
            #     self.noise_dB_span[0], self.noise_dB_span[1], 1)
            # noise_ratio = 10 ** (noise_db_rate / 20.0)
            # spk1_ref_wav = spk1_ref_wav + noise_ratio * noise_wav
            # # 挑出ref和noise,以SNR混合后重新存为wav文件,再调用pyannote读入;考虑把线索mask过程预先处理好
            # # wav_path = 'tmp_spk1_ref.wav'
            # # sf.write(wav_path, spk1_ref_wav, 16000)
            # # spk_embed = self.model({'audio': wav_path})  # pyannote对频率是否有要求?
            # # spk1_ref_vp = np.mean(spk_embed, axis=0, keepdims=True)
        # for replace
        if 'azimuth' in infer_cue_dict['replace']:
            azimuth = random.uniform(azimuth + self.azimuth_noise, azimuth - self.azimuth_noise + 2*np.pi)
            azimuth = azimuth if azimuth < 2*np.pi else azimuth - 2*np.pi  # azimuth in [0, 2*np.pi]
        if 'visual' in infer_cue_dict['replace']:
            spk1_visual = other_spk_visual
            spk1_visual = self.video_trim(spk1_visual)
        if 'voiceprint' in infer_cue_dict['replace']:
            spk1_ref_wav = other_ref_wav
            spk1_ref_wav = self.audio_trim(spk1_ref_wav)
            spk1_ref_vp = other_ref_vp
        return azimuth, spk1_visual, spk1_ref_vp, spk1_ref_wav
