...
class Trainer():
    ...
    def __init__(self,
        ...
        if QA_flag:
            ...
            quantized_module_lists = [self.net.encoder, self.net.voiceprint_encoder, self.net.visual_encoder,
                                      self.net.audio_block, self.net.fusion_block, self.net.gen_masks]
    ...
    def val(self, grid_samples):
        ...
        with torch.no_grad():
            while True:
                ...
                if self.QA_flag:
                    self.qua_op.restore_params()
                # print('loss:{} step:{}'.format(loss.item(), step))
                if len(losses) == self.logging_period:
                    avg_loss = sum(losses[-self.logging_period:]) / self.logging_period
                    self.logger.info('<epoch:{:3d}, iter:{:d}, lr:{:.3e}, loss:{:.3f}, batch:{:d} utterances> '.format(
                        self.cur_epoch, step, self.optimizer.param_groups[0]['lr'], avg_loss, len(losses)))
